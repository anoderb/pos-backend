...
async function createAdminTablesOptions() {
  ...
  const pool = new pg.Pool(
    process.env.DATABASE_URL
      ? { connectionString: process.env.DATABASE_URL, options: `-c reference=${process.env.SUPABASE_PROJECT_REF}`, ssl: { rejectUnauthorized: false } }
      : {
          ...
          port: 5432,
          database: 'postgres',
          options: `-c reference=${process.env.SUPABASE_PROJECT_REF}`,
