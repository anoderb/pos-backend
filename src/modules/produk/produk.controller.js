...
export const produkController = {
  ...
  async list(request, reply) {
    ...
    const data = await produkService.listProduk(request.toko_id, { kategori_id, stok_kritis, aktif_ai, search, pagination: request.pagination });
  ...
  async listSatuanJual(request, reply) {
    const data = await produkService.listSatuanJual(request.toko_id, request.params.id);
    return reply.send(responseSukses(data, 'Daftar satuan jual produk'));
  },

  async tambahSatuanJual(request, reply) {
    const data = await produkService.tambahSatuanJual(request.toko_id, request.params.id, request.body || {});
    return reply.code(201).send(responseSukses(data, 'Satuan jual berhasil ditambahkan'));
  },

  async updateSatuanJual(request, reply) {
    const data = await produkService.updateSatuanJual(request.toko_id, request.params.id, request.params.sid, request.body || {});
    return reply.send(responseSukses(data, 'Satuan jual berhasil diperbarui'));
  },

  async hapusSatuanJual(request, reply) {
    const data = await produkService.hapusSatuanJual(request.toko_id, request.params.id, request.params.sid);
